...
```
This is assignment 1, including chapter 1 and chapter 2. Mainly focus on the (marginal) plot and the matrix. 

The plots including two methods,with ggplot package and simply plot without ggplot. The ggplot separate the marginal plot and the main scatter plot; the plot method place all three plots together
...
ggplot(data, aes(x = Age, y = Price)) +
  geom_point() +
  labs(title = "Price vs Age", x = "Age (Years)", y = "Price (Thousands of dollars)") +
  theme_minimal()

# 2. Age Marginal dot diagram
ggplot(data, aes(x = age)) +
  geom_dotplot(binaxis = 'x', stackdir = 'center', binpositions = "all") +
...
ggplot(data, aes(x = factor(1), y = Price)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', binpositions = "all") +
  labs(title = "Price", y = "Price (Thousands of dollars)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 1),
...
library(ggplot2)
# data
data <- data.frame(sales = sales, Profits = profits)
# 1. Scatter plot
ggplot(data, aes(x = sales, y = profits))+
         geom_point()+
         labs(title = "Profit vs Sales", x = "Sales (billions)", y= "Profits (billions)") +
         theme_minimal()
# 2. Sales marginal dot plot
ggplot(data, aes(x=sales)) +
  geom_dotplot(binaxis = 'x' , stackdir = 'center', binpositions = "all") +
  labs(title = "sales", x = "Sales (billions)" ) +
  theme_minimal() +
  theme(axis.title.y=element_blank(), 
    axis.text.y=element_blank(), 
    axis.ticks.y=element_blank())
# 3. profit marginal dot plot
ggplot(data, aes(x = factor(1), y=profits)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', binpositions = "all") +
  labs(title = "Profits", y = "Profit (billions)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 1),
        axis.title.x=element_blank(), 
        axis.text.x=element_blank(), 
...
## Assets vs Sale, with ggplot
```{r}
library(ggplot2)
# data
data <- data.frame(sales = sales, assets = assets)
# 1. Scatter plot
ggplot(data, aes(x = sales, y = assets))+
         geom_point()+
         labs(title = "Assets vs Sales", x = "Assets (billions)", y= "Sales (billions)") +
         theme_minimal()
# 2. Sales marginal dot plot
ggplot(data, aes(x=sales)) +
  geom_dotplot(binaxis = 'x' , stackdir = 'center', binpositions = "all") +
  ...
  theme(axis.title.y=element_blank(), 
    ...
    axis.ticks.y=element_blank())
# 3. profit marginal dot plot
ggplot(data, aes(x = factor(1), y=assets)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', binpositions = "all") +
  labs(title = "Assets", y = "Assets (billions)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 1),
        axis.title.x=element_blank(), 
        axis.text.x=element_blank(), 
...
## Assets vs Profits, with ggplot
```{r}
library(ggplot2)
data <- data.frame(assets = assets, profits = profits)
# 1. Scatter plot
ggplot(data, aes(x = profits, y = assets))+
         geom_point()+
         labs(title = "Assets vs Profits", x = "Profits (billions)", y= "Assets (billions)") +
         theme_minimal()
# 2. profits marginal dot plot
ggplot(data, aes(x=profits)) +
  geom_dotplot(binaxis = 'x' , stackdir = 'center', binpositions = "all") +
  labs(title = "Profits", x = "Profits (billions)" ) +
  theme_minimal() +
  theme(axis.title.y=element_blank(), 
    axis.text.y=element_blank(), 
    axis.ticks.y=element_blank())
# 3. assets marginal dot plot
ggplot(data, aes(x = factor(1), y=assets)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', binpositions = "all") +
  labs(title = "Assets", y = "Assets (billions)") +
  theme_minimal() +
...
dot<- sum(x * y)
# the magnitudes of vectors x and y
magnitude_x <- sqrt(sum(x * x))
paste("the length of x is:", round(magnitude_x, 4) )
magnitude_y <- sqrt(sum(y * y))
# cosine of the angle
angle <- dot / (magnitude_x * magnitude_y)
# angle in radians
angle <- acos(angle)
paste("the angle between x and y in radian is:", round(angle, 4) )

dot_x <- sum(x * x)
scalar <- dot / dot_x
projection <- scalar * x
print("the projection of y on x is:")
round(projection, 4)
```
![ vector](ASS2-1.jpg){ width=60% }

# Question 8 (2.2)
![ vector multiplication](ASS2-2.jpg){ width=60% }
...
```{r}
A <- matrix(c(2, 1,1,3), nrow = 2, byrow = TRUE)
B <- matrix(c(1,4,2,5,0,3), nrow = 2, byrow = TRUE)
AB <- A %*% B
t(AB)
trans <- t(B) %*% t(A)
trans
...
![ vector transpose](ASS2-3.jpg){ width=60% }


# Question 10 (2.4)
![ vector transpose](ASS2-4.jpg){ width=60% }
...
solve(q)
```
In this case,  the transpose of Q is equal to its inverse. Therefore, Q is an orthogonal matrix.
